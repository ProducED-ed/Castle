...
void MapGame() {
  ...
  while (Serial2.available()) {
    ...
    // 2026-08-05: убран мгновенный Serial2-форвард cave_search1/2/3.
    // Это была ДУБЛИРУЮЩАЯ (никем не замеченная) копия старого обработчика —
    // существовала параллельно с гейтед-логикой в level==7 блоке (см. loop(),
    // ~строка 1571, фикс от 2026-07-10). Обе точки читают один Serial2, и эта
    // копия побеждала гонку за байты когда игрок проходил головоломку быстро,
    // открывая следующий этап МГНОВЕННО в обход ожидания story_27_a/b/c.
    // Симптом клиента 04.08: игрок прошёл тролля раньше чем доиграла story_26,
    // все истории тролля проигрались одна за другой без пауз.
    // Теперь форвардим находку ТОЛЬКО серверу; Serial2-ответ башне шлёт сервер
    // ПОСЛЕ окончания истории (см. buff=="cave_search1/2/3" ниже, ~строка 2822,
    // куда приходит команда от сервера и форвардится в Serial2 — этот путь
    // остаётся единственным источником открытия следующего этапа).
    if (buff == "aluminium") {
      Serial.println(F("cave_search1"));
    } else if (buff == "bronze") {
      Serial.println(F("cave_search2"));
  ...
  switch (ghostState) {
    ...
    */
    case 6:
      ...
      //
      // Датчик слушался СРАЗУ при входе в состояние. А ровно в этот момент
      // флаг ghost_knock уходит на сервер и тот запускает knock_castle.wav
      // на колонках замка — стук призрака сегодня ТОЛЬКО звук, физического
      // соленоида в библиотеке больше нет. Но звука с колонок хватает, чтобы
      // тряхнуть пьезодатчик и дать ложный punch:
      //   04:12:31  ghost_knock + PLAY knock_castle.wav
      //   04:12:35  punch        ← игрок ударить ещё не успел
      //
      // Тот же приём уже стоит на Поезде (train.ino: таймер игнорирования
      // датчика стука после команды ghost_knock) — здесь его просто не было.
      //
      // Теперь: глухое окно KNOCK_BLANK_MS после входа в состояние.
      if (knockArmedAt == 0) {
      ...
      // Проверка стука — только после того, как отзвучит knock_castle.wav.
...
const uint8_t MON_PINS[] = {
  ...
  26, 28                            // резервные входы герконов (клеммники J22/J23
                                    // на хабе) — в игре не заняты, годятся как
                                    // замена выбывшему входу
};
const uint8_t MON_DIRECT = sizeof(MON_PINS);

// Выходы Y мультиплексора. Читать их напрямую бессмысленно: значение зависит
// от того, какой канал выбран адресными линиями A/B/C. Именно поэтому в первой
// версии монитора половина датчиков «не появлялась» — герконы кубка и камина,
// сенсоры баскетбола, двери библиотеки и тюрьмы сидят как раз здесь.
const uint8_t MON_BOARDS[] = { 21, 20, A12, 33 };
...
void monPrintId(uint8_t i) {
  if (i < MON_DIRECT) {
    ...
    Serial.print('b');
    Serial.print(e / MON_CHANNELS + 1);
    Serial.print('c');
    Serial.print(e % MON_CHANNELS);
  }
}
...
uint8_t monRead(uint8_t i) {
  if (i < MON_DIRECT) return digitalRead(MON_PINS[i]);
  if (i == MON_KNOCK_IDX) {
    // Стук — импульс, а не уровень. Держим «сработал» коротким окном, иначе
    // одиночный удар мелькнёт быстрее, чем человек успеет посмотреть на экран.
...
// Пауза на стабилизацию адреса мультиплексора. Была delay(1) — целая
// миллисекунда, и это единственное, что делает опрос дорогим.
//
// 2026-08-15, CLC4: на этапе Мастерской цикл платы шёл 1444 мкс вместо 58 —
// Oven() опрашивает расширитель каждый проход, и почти всё время уходило в
// этот сон. При таком цикле худшая итерация доходила до 24 мс, а за 24 мс на
// 115200 в буфер приёма (64 байта) прилетает вчетверо больше, чем он держит:
// команда с сервера теряется. Ровно так в тот день пропал soundoff, и
// подсказки умерли до конца этапа.
//
// 200 мкс — с четырёхкратным запасом к постоянной времени линии
// (подтяжка 10 кОм на плате расширителя, ёмкость шлейфа): пять tau
// укладываются в полсотни микросекунд. Если после этой правки какой-то датчик
// начнёт врать — поднимать надо именно эту константу, она здесь одна.
const unsigned int EXPANDER_SETTLE_US = 200;

bool digitalReadExpander(int pin, int boardNumber) {
  // Пауза нужна только когда адрес МЕНЯЕТСЯ. Игровые функции опрашивают один
  // и тот же канал подряд тысячи раз — там ждать нечего, выход мультиплексора
  // давно установился. Платы board1..board4 сидят на общих адресных линиях и
  // отличаются только пином данных, поэтому ключ кэша — адрес, а не плата.
  ...
  if (pin != lastAddr) {
    ...
    lastAddr = pin;
    delayMicroseconds(EXPANDER_SETTLE_US);
  }
