...
// === DIAG MODE (минимальный, char-only — не String, чтобы не фрагментировать heap) ===
// Server -> Mega (Serial0):
//   DIAG_ON:<device>            (device: main|workshop|basket|dog|owls)
//   DIAG_OFF:<device>
//   DIAG_SET:<device>:<key>:<val>
// Для башен Mega пересылает в их UART как DIAG_ON / DIAG_OFF / DIAG_SET:<key>:<val>
// Mega -> Server (Serial0):
//   DIAG_SNAPSHOT:main:{...JSON...}        (5 раз/сек, пока diagMainActive)
//   DIAG_SNAPSHOT:<tower>:{...JSON...}     (отретранслированный snapshot от башни)
...
void loop() {
  ...
  // === DIAG: перехват D-команд от сервера через Serial0 (всегда работает) ===
  // Игровые команды все в lower-case, поэтому 'D' на peek — это DIAG_*.
  while (Serial.available() && Serial.peek() == 'D') {
    ...
    while (millis() - t0 < 200UL) {
      if (Serial.available()) {
        char cc = Serial.read();
        if (cc == '\n' || cc == '\r') break;
        if (diagInLen < sizeof(diagInBuf) - 1) diagInBuf[diagInLen++] = cc;
      }
    }
    diagInBuf[diagInLen] = 0;
    if (strncmp(diagInBuf, "DIAG_", 5) == 0) processDiagCmd(diagInBuf);
    diagInLen = 0;
  }

  // === DIAG: если в diag-режиме — relay snapshot'ов башен + наш snapshot, и НЕ запускаем игровую логику
  if (diagMainActive) {
    relayTowerSnapshots();
    if (millis() - diagLastMainSnap >= 200UL) {
      diagLastMainSnap = millis();
      sendDiagSnapMain();
    }
    return;
  }
...
// ====================================================================
// === DIAG MODE: implementation (char-only, без String) ===
// ====================================================================

void diagSetMainOutput(const char* key, int val) {
  if (strcmp(key, "mansard_door") == 0)    { OpenDoor(MansardDoor); return; }
  if (strcmp(key, "potions_door") == 0)    { OpenDoor(PotionsRoomDoor); return; }
  if (strcmp(key, "library_door") == 0)    { OpenDoor(LibraryDoor); return; }
  if (strcmp(key, "bank_door") == 0)       { OpenDoor(BankDoor); return; }
  if (strcmp(key, "low_tower_door") == 0)  { OpenDoor(HightTowerDoor); return; }
  if (strcmp(key, "high_tower_door") == 0) { OpenDoor(HightTowerDoor2); return; }
  if (strcmp(key, "memory_door") == 0)     { OpenDoor(MemoryRoomDoor); return; }
  if (strcmp(key, "crime_door") == 0)      { OpenDoor(CrimeDoor); return; }
  if (strcmp(key, "stash_door") == 0)      { OpenDoor(BankStashDoor); return; }
  if (strcmp(key, "fireworks") == 0)       { digitalWrite(Fireworks, val ? HIGH : LOW); return; }
}
...
static Stream* diagTowerStream(int8_t idx) {
  if (idx == 0) return &Serial1;
  if (idx == 1) return &Serial2;
  if (idx == 2) return &Serial3;
  if (idx == 3) return &mySerial;
  return nullptr;
}

void processDiagCmd(char* cmd) {
  // DIAG_ON:<device>
  if (strncmp(cmd, "DIAG_ON:", 8) == 0) {
    char* dev = cmd + 8;
    int8_t idx = diagTowerIndex(dev);
    if (idx == -1) {
      diagMainActive = true;
      for (int i = 0; i < DOORS; i++) { digitalWrite(doors[i], LOW); active[i] = false; }
      digitalWrite(Fireworks, LOW);
      Serial.println(F("DIAG_ACK_MAIN:ON"));
    } else if (idx >= 0) {
      // Любая башня в diag-режиме требует, чтобы Mega тоже была заморожена
      // (иначе её state-machine будет дёргать Serial1/2/3/mySerial).
      if (!diagMainActive) {
        diagMainActive = true;
        for (int i = 0; i < DOORS; i++) { digitalWrite(doors[i], LOW); active[i] = false; }
        digitalWrite(Fireworks, LOW);
      }
      Stream* s = diagTowerStream(idx);
      if (s) s->println(F("DIAG_ON"));
    }
    return;
  }
  // DIAG_OFF:<device>
  if (strncmp(cmd, "DIAG_OFF:", 9) == 0) {
    char* dev = cmd + 9;
    int8_t idx = diagTowerIndex(dev);
    if (idx == -1) {
      diagMainActive = false;
      Serial.println(F("DIAG_ACK_MAIN:OFF"));
    } else if (idx >= 0) {
      Stream* s = diagTowerStream(idx);
      if (s) s->println(F("DIAG_OFF"));
    }
    return;
  }
  // DIAG_SET:<device>:<key>:<val>
  if (strncmp(cmd, "DIAG_SET:", 9) == 0) {
    char* dev = cmd + 9;
    char* colon = strchr(dev, ':');
    if (!colon) return;
    *colon = 0;
    char* rest = colon + 1;  // <key>:<val>
    int8_t idx = diagTowerIndex(dev);
    if (idx == -1) {
      // main: разбираем <key>:<val>
      char* c2 = strchr(rest, ':');
      int val = 0;
      if (c2) { *c2 = 0; val = atoi(c2 + 1); }
      diagSetMainOutput(rest, val);
    } else if (idx >= 0) {
      Stream* s = diagTowerStream(idx);
      if (s) { s->print(F("DIAG_SET:")); s->println(rest); }
    }
    return;
  }
}
...
void sendDiagSnapMain() {
  unsigned long now = millis();
  unsigned long hw = lastSeenWorkshop ? (now - lastSeenWorkshop) : 999999UL;
  unsigned long hb = lastSeenBasket   ? (now - lastSeenBasket)   : 999999UL;
  unsigned long hd = lastSeenDog      ? (now - lastSeenDog)      : 999999UL;
  unsigned long ho = lastSeenOwls     ? (now - lastSeenOwls)     : 999999UL;
  // Формат: DIAG_SNAPSHOT:main:{"in":{...},"volt":N,"hb":{...},"up":N}
  Serial.print(F("DIAG_SNAPSHOT:main:{\"in\":{\"cr1\":"));
  Serial.print(digitalRead(firstCrystal));
  Serial.print(F(",\"cr2\":"));
  Serial.print(digitalRead(secondCrystal));
  Serial.print(F(",\"cr3\":"));
  Serial.print(digitalRead(thirdCrystal));
  Serial.print(F(",\"cr4\":"));
  Serial.print(digitalRead(fourCrystal));
  Serial.print(F(",\"win\":"));
  Serial.print(analogRead(WindowSens) > 500 ? 1 : 0);
  Serial.print(F(",\"crm\":"));
  Serial.print(digitalRead(crimeDoorPin));
  Serial.print(F(",\"strt\":"));
  Serial.print(digitalRead(startDoorPin));
  Serial.print(F("},\"volt\":"));
  Serial.print(analogRead(voltagePin));
  Serial.print(F(",\"hb\":{\"workshop\":"));
  Serial.print(hw);
  Serial.print(F(",\"basket\":"));
  Serial.print(hb);
  Serial.print(F(",\"dog\":"));
