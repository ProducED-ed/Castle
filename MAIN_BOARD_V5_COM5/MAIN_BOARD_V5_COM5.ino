...
// Сервер шлёт по Serial0 UPPERCASE-команды (отличается от game-команд в lowercase):
//   DIAG_ON:<device>            — войти в diag для main | workshop | basket | dog | owls
//   DIAG_OFF:<device>           — выйти
//   DIAG_SET:<device>:<k>:<v>   — изменить выход
// Mega шлёт обратно: DIAG_SNAPSHOT:<device>:<JSON> каждые 200ms.
// Для main: snapshot всех своих I/O + воспроизведение Mega-стороной.
// Для tower: Mega проксирует команды в Serial1/2/3/mySerial и форвардит
// snapshot-строки от башен обратно на Serial0.
bool diagMainActive = false;
bool diagTowerActive[4] = {false, false, false, false};  // workshop, basket, dog, owls
unsigned long diagLastMainSnapshot = 0;
// Буфер для накопления uppercase-команды от сервера, чтобы не мешать game-state readers
String diagInputBuf = "";
bool diagInputBufActive = false;
// Буферы для строк от каждой башни (на случай если строка приходит фрагментами)
String diagTowerBuf[4] = {"", "", "", ""};
void handleDiagLine(String line);
...
void loop() {
  // === DIAG MODE: перехват uppercase DIAG_* команд от сервера ===
  // Накопляем буфер ТОЛЬКО если первый байт = 'D' (uppercase) — иначе
  // оставляем для game-state readers (PowerOn / level handlers / etc).
  {
    while (Serial.available()) {
      int p = Serial.peek();
      if (!diagInputBufActive && p != 'D') break;  // не наша команда, отдаём game-логике
      int ch = Serial.read();
      if (ch == '\n' || ch == '\r') {
        if (diagInputBuf.length() > 0) {
          String line = diagInputBuf;
          line.trim();
          handleDiagLine(line);
          diagInputBuf = "";
        }
        diagInputBufActive = false;
      } else {
        diagInputBuf += (char)ch;
        diagInputBufActive = true;
        if (diagInputBuf.length() > 250) {  // защита от мусора
          diagInputBuf = "";
          diagInputBufActive = false;
        }
      }
  ...
  // Если активна диагностика главной платы — пропускаем игровую логику
  if (diagMainActive) {
    if (millis() - diagLastMainSnapshot >= 200UL) {
      diagLastMainSnapshot = millis();
      sendDiagSnapshotMain();
    }
    // Проверим tower-snapshot'ы даже когда main в diag — на случай если оба
    // diag-режима активны одновременно (можно, см. [[clc-tech-pult-diag-panel]])
    relayTowerDiagSnapshots();
    return;
  }
  // Если есть активные tower-diag, тоже шлём snapshot башен на сервер
  // (но game-логика продолжает работать)
  relayTowerDiagSnapshots();
  ...
  if (millis() - lastHbLog > 60000UL) {
    lastHbLog = millis();
    unsigned long now = millis();
    Serial.print(F("log:main:HB W="));
    Serial.print(lastSeenWorkshop ? (now - lastSeenWorkshop) : 999999UL);
    Serial.print(F(" B="));
    Serial.print(lastSeenBasket ? (now - lastSeenBasket) : 999999UL);
    Serial.print(F(" D="));
    Serial.print(lastSeenDog ? (now - lastSeenDog) : 999999UL);
    Serial.print(F(" O="));
...
void PowerOn() {
  ...
  while (Serial.available()) {
    ...
    if (buff == "start") {
      ...
      Serial.print((lastSeenWorkshop && (now - lastSeenWorkshop) < w) ? 1 : 0);
      Serial.print(F(",basket="));
      Serial.print((lastSeenBasket && (now - lastSeenBasket) < w) ? 1 : 0);
      Serial.print(F(",dog="));
      Serial.print((lastSeenDog && (now - lastSeenDog) < w) ? 1 : 0);
      Serial.print(F(",owls="));
...
// =====================================================
// === DIAG MODE helpers (см. [[clc-tech-pult-diag-panel]])
// =====================================================

// Парсит и обрабатывает строку DIAG_* от сервера
void handleDiagLine(String line) {
  // line примеры:
  //   "DIAG_ON:main"        "DIAG_OFF:main"
  //   "DIAG_ON:workshop"    "DIAG_OFF:workshop"
  //   "DIAG_SET:main:open_mansard:0"  (key:val, key содержит подчёркивания)
  //   "DIAG_SET:workshop:foo_bar:1"   (для tower просто пробрасываем как "DIAG_SET:foo_bar:1")

  // device-index helper: 0..3 = workshop/basket/dog/owls, -1 = main
  auto deviceIndex = [](const String& d) -> int {
    if (d == "workshop") return 0;
    if (d == "basket")   return 1;
    if (d == "dog")      return 2;
    if (d == "owls")     return 3;
    return -1;  // main или неизвестное
  };

  auto sendToTower = [](int idx, const String& msg) {
    if (idx == 0) Serial1.println(msg);
    else if (idx == 1) Serial2.println(msg);
    else if (idx == 2) Serial3.println(msg);
    else if (idx == 3) mySerial.println(msg);
  };

  if (line.startsWith("DIAG_ON:")) {
    String dev = line.substring(strlen("DIAG_ON:"));
    int idx = deviceIndex(dev);
    if (idx == -1 && dev == "main") {
      diagMainActive = true;
      // Гасим все выходы Mega
      ...
      digitalWrite(BankRoomLight, LOW);
      digitalWrite(TorchLight, LOW);
      Serial.println("log:DIAG main ON");
    } else if (idx >= 0) {
      diagTowerActive[idx] = true;
      sendToTower(idx, "DIAG_ON");
      Serial.print(F("log:DIAG tower "));
      Serial.print(dev);
      Serial.println(F(" ON (relayed)"));
    }
  }
  else if (line.startsWith("DIAG_OFF:")) {
    String dev = line.substring(strlen("DIAG_OFF:"));
    int idx = deviceIndex(dev);
    if (idx == -1 && dev == "main") {
      diagMainActive = false;
      Serial.println("log:DIAG main OFF");
    } else if (idx >= 0) {
      diagTowerActive[idx] = false;
      sendToTower(idx, "DIAG_OFF");
      Serial.print(F("log:DIAG tower "));
      Serial.print(dev);
      Serial.println(F(" OFF (relayed)"));
    }
  }
  else if (line.startsWith("DIAG_SET:")) {
    // DIAG_SET:<device>:<rest>
    String rest = line.substring(strlen("DIAG_SET:"));
    int colon = rest.indexOf(':');
    if (colon < 0) return;
    String dev = rest.substring(0, colon);
    String body = rest.substring(colon + 1);  // "<key>:<val>"
    int idx = deviceIndex(dev);
    if (idx == -1 && dev == "main") {
      handleDiagSetMain(body);
    } else if (idx >= 0) {
      sendToTower(idx, "DIAG_SET:" + body);
    }
  }
}

void handleDiagSetMain(String body) {
  // body = "<key>:<val>"
  ...
  // --- Двери (импульсные, 300мс HIGH потом LOW) ---
  if      (key == "door_mansard")    OpenDoor(MansardDoor);
  else if (key == "door_potions")    OpenDoor(PotionsRoomDoor);
  else if (key == "door_library")    OpenDoor(LibraryDoor);
  else if (key == "door_bank")       OpenDoor(BankDoor);
  else if (key == "door_high_tower") OpenDoor(HightTowerDoor);
  else if (key == "door_high_tower2") OpenDoor(HightTowerDoor2);
  else if (key == "door_memory")     OpenDoor(MemoryRoomDoor);
  else if (key == "door_crime")      OpenDoor(CrimeDoor);
  else if (key == "door_bank_stash") OpenDoor(BankStashDoor);
  // --- Свет (toggle ON/OFF) ---
  else if (key == "library_light")   digitalWrite(LibraryLight, onv);
  else if (key == "hall_light")      digitalWrite(HallLight, onv);
  else if (key == "uf_hall_light")   digitalWrite(UfHallLight, onv);
  else if (key == "mansard_light")   digitalWrite(MansardLight, onv);
  else if (key == "last_tower_light") digitalWrite(LastTowerTopLight, onv);
  else if (key == "fireworks")       digitalWrite(Fireworks, onv);
  else if (key == "bank_room_light") digitalWrite(BankRoomLight, onv);
  else if (key == "torch_light")     digitalWrite(TorchLight, onv);
  // --- Соленоид стука (100мс импульс) ---
  else if (key == "knock_pulse") {
    digitalWrite(KnockSol, HIGH);
    smartDelay(100);
    digitalWrite(KnockSol, LOW);
  }
}

// Snapshot всех I/O главной платы → DIAG_SNAPSHOT:main:{...}
void sendDiagSnapshotMain() {
  // Сенсоры (1 = активен/замкнут)
  // Все INPUT_PULLUP → нажат/замкнут = LOW → инвертируем для UI
  String s = "DIAG_SNAPSHOT:main:{\"in\":[";
  s += (digitalRead(firstCrystal)  == LOW ? "1" : "0"); s += ",";
  s += (digitalRead(secondCrystal) == LOW ? "1" : "0"); s += ",";
  s += (digitalRead(thirdCrystal)  == LOW ? "1" : "0"); s += ",";
  s += (digitalRead(fourCrystal)   == LOW ? "1" : "0"); s += ",";
  s += (digitalRead(WindowSens)    == LOW ? "1" : "0"); s += ",";
  s += (digitalRead(crimeDoorPin)  == LOW ? "1" : "0"); s += ",";
  s += (digitalRead(startDoorPin)  == LOW ? "1" : "0");
  s += "],\"v\":";
  s += String(analogRead(voltagePin));  // 0-1023, для пьезо/напряжения
  // Выходы — состояние пинов (digital read возвращает заnish-записанное значение для OUTPUT)
  s += ",\"out\":[";
  s += String(digitalRead(LibraryLight));     s += ",";
  s += String(digitalRead(HallLight));        s += ",";
  s += String(digitalRead(UfHallLight));      s += ",";
  s += String(digitalRead(MansardLight));     s += ",";
  s += String(digitalRead(LastTowerTopLight)); s += ",";
  s += String(digitalRead(Fireworks));        s += ",";
  s += String(digitalRead(BankRoomLight));    s += ",";
  s += String(digitalRead(TorchLight));
  s += "],\"twr\":[";
  // Башни alive (по lastSeen* — 0 если не отвечает > 15с)
  unsigned long now = millis();
  unsigned long w = 15000UL;
  s += ((lastSeenWorkshop && (now - lastSeenWorkshop) < w) ? "1" : "0"); s += ",";
  s += ((lastSeenBasket   && (now - lastSeenBasket)   < w) ? "1" : "0"); s += ",";
  s += ((lastSeenDog      && (now - lastSeenDog)      < w) ? "1" : "0"); s += ",";
  s += ((lastSeenOwls     && (now - lastSeenOwls)     < w) ? "1" : "0");
  s += "],\"up\":";
  s += String(now / 1000UL);
  s += ",\"lvl\":";
  s += String((int)level);
...
// Читаем из всех башен потенциальные DIAG_SNAPSHOT:... строки, оборачиваем
// и шлём на сервер. Параллельно — не съедаем не-snapshot данные (они нужны
// для HelpTowersHandler и игровой логики).
void relayTowerDiagSnapshots() {
  static String towerNames[4] = {"workshop", "basket", "dog", "owls"};
  for (int i = 0; i < 4; i++) {
    if (!diagTowerActive[i]) continue;  // башня не в diag — не читаем
    Stream* src = nullptr;
    if (i == 0) src = &Serial1;
    else if (i == 1) src = &Serial2;
    else if (i == 2) src = &Serial3;
    else if (i == 3) src = &mySerial;
    while (src->available()) {
      char c = (char)src->read();
      if (c == '\n' || c == '\r') {
        if (diagTowerBuf[i].length() > 0) {
          diagTowerBuf[i].trim();
          if (diagTowerBuf[i].startsWith("DIAG_SNAPSHOT:")) {
            // Перепаковываем: DIAG_SNAPSHOT:<rest> → DIAG_SNAPSHOT:<tower>:<rest>
            String body = diagTowerBuf[i].substring(strlen("DIAG_SNAPSHOT:"));
            Serial.print(F("DIAG_SNAPSHOT:"));
            Serial.print(towerNames[i]);
            Serial.print(':');
            Serial.println(body);
          }
          // Не-snapshot строки (pong и т.п.) ИГНОРИРУЕМ — башня в diag не должна слать игровых
          diagTowerBuf[i] = "";
        }
      } else {
        diagTowerBuf[i] += c;
        if (diagTowerBuf[i].length() > 400) diagTowerBuf[i] = "";  // защита
      }
    }
  }
}
