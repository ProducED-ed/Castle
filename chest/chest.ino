...
void loop() {
  ...
  if(helpButton.isPress()){
    ...
    // 2026-08-14: было state > 3, и состояние 3 не ловила ни одна ветка —
    // ровно та же дыра, что нашлась у Волка. А состояние 3 это «этап пройден,
    // играет финальный джингл», то есть именно тот момент, когда игрок должен
    // услышать выделенную подсказку про пройденный уровень. Замок при этом
    // открывается по таймеру, а не по концу трека, так что перебить джингл
    // подсказкой безопасно.
    if(state >= 3 && hintFlag){
      if(language == 1){
 ...
 switch (state) {
   ...
   case 3:
      ...
      // 2026-08-05: сокращено с 5000 до 2000мс. Жалоба клиента: головоломка
      // решена (индикатор на пульте зелёный), но сундук "не открывался
      // автоматически" — на деле открывался, но через полные 5 сек, потому
      // что DFPlayer-событие finishedTrack==TRACK_SUITCASE_END ненадёжно
      // (тот же класс бага что уже чинили на Safe, см. память clc-safe-
      // dfplayer-fix) и почти всегда срабатывал именно safety-таймаут,
      // не событие. 5 сек ощущались как "не работает". 2 сек — компромисс:
      // короче для игрока, но не обрезает большинство коротких success-джинглов.
      if (millis() - timerEndLed > 2000) {
...
void SendData(){
  if (WiFi.status() == WL_CONNECTED) {
    sendLogToServer("{\"log\":\"Suitcase game finished, sending 'end' to server.\"}");
    HTTPClient http;
...
void EasyButtons() {
  ...
  // 18.08.2026: зажигаем ВСЕ четыре символа, а не только свой.
  // Лёгкий режим для команд, которые буксуют: одно нажатие — и головоломка
  // визуально решена целиком. Раньше загорался символ нажатой кнопки, и игроки
  // всё равно шли искать остальные три, то есть режим помогал только наполовину.
  // Нажатия остальных кнопок после этого ничего не меняют: всё уже горит.
  for (int i = 0; i <= 255; i++) {
    for (int k = 0; k < 4; k++) analogWrite(ledsSym[k], i);
    delay(3);
  }
