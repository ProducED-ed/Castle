#!/usr/bin/env python3
"""ПЕРВИЧНЫЙ импорт текстов подсказок из Google-таблицы в hint_texts.json.

⚠ Решение Эдуарда 17.08.2026: таблица была нужна один раз, чтобы взять тексты.
Дальше источник правды — **файл на самом замке**, как сервер, шаблоны и
прошивки. У разных клиентов формулировки могут расходиться, и подтягивать их
из общей таблицы неправильно.

Поэтому:
  * этот скрипт — для НОВОГО замка или если нужно пересобрать эталон с нуля;
  * `hint_texts.json` в репозитории — эталон для нового замка, а не хозяин;
  * обычная раскатка тексты НЕ трогает (см. tools/rollout_deploy.py), заменить
    их на замке можно только осознанно, флагом --texts;
  * правку под конкретного клиента делаем прямо в его файле на замке.

    python3 tools/export_hint_texts.py          # перезаписать эталон
