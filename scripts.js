...
//флаги
 ...
 //настройка подключения не менять
     ...
     socket.on('connect', function() {//если наш сервер подключен он отправит сообщение коннектед
         ...
         // 2026-07-31: сброс флагов-защёлок перед повторным проигрыванием истории.
         // При КАЖДОМ подключении сервер шлёт весь socklist заново. Блоки, которые
         // разом гасят игровые кнопки, стоят под защитой `if (disBut == 0)` и
         // `if (restflag == 0)` — то есть отрабатывают ОДИН раз за сессию страницы.
         // После обрыва связи (Wi-Fi glitch, рестарт сервера) флаги оставались
         // взведёнными, массовое гашение пропускалось, а события active_* только
         // СНИМАЛИ disabled. В итоге часть кнопок оставалась активной, хотя этап
         // ещё не открыт — и лечилось только F5, который обнулял переменные.
         // Обнуляем их здесь, чтобы replay истории отработал как при первой загрузке.
         disBut = 0;
         restflag = 0;
