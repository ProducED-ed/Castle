...
void loop() {
  ...
  if (flagButton.isPress()) {
    ...
    _restartFlag = 1;   // держим собственное состояние в согласии с отправленным
  }
  ...
  // Сообщения уходят только на фронтах, и потерянный фронт разводил картину
  // Mega с реальностью НАВСЕГДА — клиент лечил это, вынимая и вставляя флаг.
  // Mega форвардит наверх только изменения, так что повтор ничего не засоряет.
  //
  // 20.08.2026: сам повтор был написан ещё 07.08, но стоял ВНУТРИ обработчика
  // команды restart — то есть во время игры не выполнялся ни разу. У Корзины он
  // с самого начала в loop(), у Сов и Цербера — в CheckState(), которую зовут
  // постоянно. Мастерская осталась единственной башней без работающего повтора,
  // и именно её флаг (красный) клиент в Канаде жал по два раза.
  // Состояние читаем с пина, а не из переменной: пин — источник правды.
  if (millis() - lastFlagResend >= FLAG_RESEND_MS) {
    lastFlagResend = millis();
    Serial1.println(digitalRead(FLAG_IR_SENSOR_PIN) ? "flag1_on" : "flag1_off");
  }
...
void handleUartCommands() {
  if (Serial1.available()) {
    ...
    if (command == "workshop") {
      ...
      _restartGalet = 0;
      _restartFlag = 0;
      // На рестарте отдаём состояние принудительно, не дожидаясь повтора:
      // Mega только что обнулила свою картину, и ей нужны обе строки сразу.
      // Периодический повтор живёт в loop(), здесь ему было не место —
      // во время игры эта ветка не выполняется.
      CheckState(true);
      lockOpen = false;
      recurringLockActive = false;
