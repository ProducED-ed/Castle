...
void wsDiagHandleLine(String line) {
  if (line == "DIAG_ON") {
    ...
    digitalWrite(LED_FLOOR2_PIN, LOW);
    digitalWrite(ROOF_LIGHT_PIN, LOW);
    for (int i = 0; i < NUM_LEDS_FIRE; i++) strip.setPixelColor(i, 0);
    strip.show();
    ...
    String val = s.substring(c + 1);
    int onv = val.toInt() ? HIGH : LOW;
    if      (key == "led_floor1") digitalWrite(LED_FLOOR1_PIN, onv);
    else if (key == "led_floor2") digitalWrite(LED_FLOOR2_PIN, onv);
    else if (key == "roof_light") digitalWrite(ROOF_LIGHT_PIN, onv);
    else if (key == "lock_pulse") {
      digitalWrite(LOCK_PIN, HIGH);
      delay(500);
      digitalWrite(LOCK_PIN, LOW);
    } else if (key == "np_fire" || key == "np_workbench") {
      long n = strtol(val.c_str(), NULL, 16);
      uint8_t r = (n >> 16) & 0xFF, g = (n >> 8) & 0xFF, b = n & 0xFF;
      if (key == "np_fire") {
        for (int i = 0; i < NUM_LEDS_FIRE; i++) strip.setPixelColor(i, r, g, b);
        strip.show();
      } else {
        for (int i = 0; i < NUM_LEDS_WORKBENCH; i++) workbenchStrip.setPixelColor(i, r, g, b);
        workbenchStrip.show();
      }
    }
  }
...
void wsSendDiagSnapshot() {
  // in[]: 8 sensors — все HIGH_PULL гёрконы/кнопки active=LOW; ИК флаг и галетник — active=LOW
  String s = "DIAG_SNAPSHOT:{\"in\":[";
  s += (digitalRead(REED_SWITCH_PIN)        == LOW ? "1" : "0"); s += ",";
  s += (digitalRead(HINT_REED_SWITCH_PIN)   == LOW ? "1" : "0"); s += ",";
  s += (digitalRead(REED_WORKBENCH1_PIN)    == LOW ? "1" : "0"); s += ",";
  s += (digitalRead(REED_WORKBENCH2_PIN)    == LOW ? "1" : "0"); s += ",";
  s += (digitalRead(REED_WORKBENCH3_PIN)    == LOW ? "1" : "0"); s += ",";
  s += (digitalRead(REED_WORKBENCH4_PIN)    == LOW ? "1" : "0"); s += ",";
  s += (digitalRead(GALET_PIN)              == LOW ? "1" : "0"); s += ",";
  s += (digitalRead(FLAG_IR_SENSOR_PIN)     == LOW ? "1" : "0");
  s += "],\"out\":[";
  s += String(digitalRead(LED_FLOOR1_PIN));  s += ",";
  s += String(digitalRead(LED_FLOOR2_PIN));  s += ",";
  s += String(digitalRead(ROOF_LIGHT_PIN));
  s += "],\"up\":";
