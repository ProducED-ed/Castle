...
5. Запустить first-boot:
   ...
   # → задаёт hostname (например castle-clc-4)
   # То же самое можно сделать с тех-пульта: /tech → «Первый запуск» → шаг 1
   ```
6. Проверить что:
...
**Основной путь — с тех-пульта:** `/tech` → «Первый запуск» → шаг «Tailscale» → «Включить и
авторизовать». Ссылку авторизации пульт покажет сам, её можно скопировать на устройство с интернетом.
Если есть pre-auth key из админки — вписать в поле, тогда всё пройдёт без ссылки.

Из консоли, если пульт недоступен:
...
Tailscale возьмёт имя квеста (задано в `first-boot-clc.sh` или на пульте).
