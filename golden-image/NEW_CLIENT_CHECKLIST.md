...
- [ ] `clc-hostname.txt` с одной строкой имени, например:
  ...
  castle-clc-4
  ```
  Формат: `castle-clc-N`, только латиница a-z 0-9 дефис, БЕЗ заглавных, БЕЗ подчёркиваний. Тогда `first-boot-clc.sh` не будет спрашивать. Это же имя видно в Tailscale и в реестре клиентов.

  Можно пропустить: имя задаётся с тех-пульта (`/tech` → «Первый запуск» → шаг 1).
...
### 2.2. Имя квеста (если hostname не задан в шаге 1.3)

С тех-пульта, консоль не нужна:
- [ ] `http://192.168.4.1:3000/tech` → пароль `questquest`
- [ ] Раздел **«Первый запуск» → шаг 1 «Имя квеста»** → ввести `castle-clc-N` → «Применить»

Запасной путь, если пульт не открывается — SSH (Castle WiFi → `pi@192.168.4.1`, пароль `questquest`):
`sudo bash /usr/local/sbin/first-boot-clc.sh`

### 2.3. Раскладка USB-хаба (проверить обязательно!)

Эталон для всех новых замков — втыкать башни в хаб **в этом порядке**:
...
⚠️ Если перепутать — «Прошить Workshop» уйдёт физически в Basket, прошивка пройдёт «успешно», а башня будет вести себя странно. Ровно это было на CLC3.

Проверка с пульта, вручную сверять пути больше не надо:
- [ ] **«Первый запуск» → шаг «Раскладка USB-хаба»** → «Опознать башни»
- [ ] Все четыре порта зелёные? Если красный — поменять кабели местами по подсказке и опознать заново
- [ ] Нажать «Подтвердить раскладку»

Если замок уже собран иначе и перетыкать нельзя — там же «раскладка отличается от эталона» и назначить вручную.
...
### 3.1. Подключение wlan1 к сети клиента

Всё с пульта: подключиться к сети **Castle**, открыть `/tech` → **«Первый запуск» → шаг «WiFi для интернета»**.

- [ ] «Найти сети» → выбрать сеть клиента из списка (вручную SSID вводить не надо — меньше шансов ошибиться)
- [ ] Ввести пароль → «Подключить» → дождаться IP
- [ ] ⚠️ **Красный бейдж WPA3** — донгл MT7601U и подобные такую сеть не видят (см. [mt7601u-wpa3-blind]). Просить у клиента **парную WPA2-сеть** или перевести роутер в WPA2-mode.
- [ ] ⚠️ **Оранжевый бейдж 5 ГГц** — донгл может не поддерживать. При раздаче с телефона переключить точку на 2.4 ГГц.

### 3.2. Tailscale up

Там же, шаг «Tailscale»:

- [ ] «Включить и авторизовать». Если есть pre-auth key из админки — вписать в поле, тогда всё пройдёт само
- [ ] Без ключа — на пульте появится ссылка авторизации. Скопировать её на устройство с интернетом и открыть
- [ ] Дождаться статуса Running и Tailscale IP на пульте
- [ ] В админке https://login.tailscale.com/admin/machines найти ноду
- [ ] ⚠️ **ОБЯЗАТЕЛЬНО**: меню `⋯` → **Disable key expiry** (иначе через 6 мес. потеряет IP!) → отметить галочку на пульте
...
- [ ] **Раскладка USB-хаба**: «эталон» или, если отличается, какая именно (из шага 2.3)
- [ ] **Калибровка серво мальчика**: четыре значения из шага «Калибровка серво» на пульте
...
| Workshop/Basket прошился но молчит | Раскладка USB-хаба свапнута — «Первый запуск» → «Опознать башни» (см. шаг 2.3) |
| Мальчик не доезжает / упирается | Калибровка серво — «Первый запуск» → «Калибровка серво», кнопки «Спрятать»/«Открыть» |
