...
// 14.08.2026, после перезаписи файлов: все языки ровно 13 секунд, поэтому
// запас теперь не двойной, а в две секунды. Когда событие «трек доиграл»
// приходит штатно (около 13 с), игра стартует по нему; когда теряется —
// на пятнадцатой секунде. Разницы на слух уже не будет.
const unsigned long STORY15_MAX_MS  = 15000UL;  // история 13 с + 2 с
