...
// 14.08.2026. Длительности измерил Эдуард: train_on (трек 1) — 4 с; story_15 по
// языкам — 12, 12, 13, 9, 8, 7 с, то есть максимум 13. Прежние 20 и 60 секунд
// брались с тройным запасом «на глаз», и когда DFPlayer терял событие конца
// трека — а он теряет его регулярно, — этап поезда начинался через полторы
...
// 14.08.2026, решение Эдуарда: игра стартует строго по таймеру — 13,5 секунды
// от начала истории. Все языки ровно 13 секунд, так что полсекунды это пауза
// на вдох, а не ожидание. Событие «трек доиграл» от DFPlayer никуда не делось
// и, если приходит, стартует игру чуть раньше — но полагаться на него больше
// не приходится: оно теряется регулярно.
const unsigned long STORY15_MAX_MS  = 13500UL;  // история 13 с + 0,5 с
