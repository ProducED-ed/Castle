...
const uint8_t MON_PINS[] = {
  ...
  A0, A1,            // A0 — геркон вращения платформы (скорость), A1 — КОНЕЧНЫЙ
                     // геркон: платформа защёлкнулась, этап засчитан. Замок
                     // клетки сюда не относится, это выход D5.
...
void loop() {
  ...
  if (currentQuestState != previousState) {
    ...
    // RESTART mariyu WAITING logs ni block chestunnamu
    if (currentQuestState != STATE_RESTARTING && currentQuestState != STATE_WAITING_FOR_START) {
