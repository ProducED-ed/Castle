...
# до ~2 секунд eventlet.sleep(). Всё это время channel3.get_busy() == False, и
...
def story_busy():
    ...
    Везде, где раньше стояло channel3.get_busy(), нужен именно этот ответ:
    «прозвучит ли сейчас чужая история, если я запущу свою».
    """
    return channel3.get_busy() or story_play_pending > 0
...
# 14.08.2026: смысл тумблера перевёрнут — включённый теперь означает ЛЁГКИЙ
# режим, как у Чемоданов. Файл нарочно НОВЫЙ: в старом единица означала
# «сложный», и если его перечитать, замок понял бы её наоборот.
WOLF_PREF_FILE = '/home/pi/castle_wolf_easy_pref'

def _load_wolf_easy_preference():
    """Дефолт — False (нормальный режим: откат луны через 5 с)."""
    try:
...
def _save_wolf_easy_preference(state):
    try:
...
def _push_wolf_mode():
    ...
    cmd = "wolf_mode_easy" if wolf_easy_mode else "wolf_mode_normal"
    send_esp32_command(ESP32_API_WOLF_URL, cmd)

wolf_easy_mode = _load_wolf_easy_preference()
...
def gather_system_status():
    ...
    #
    # 19.08.2026: пробовал поднять таймаут до 3 с — думал, что проба не
    # переживает подъём сотового радио и потому шлёт ложные тревоги на раздаче
    # с телефона. Замер опроверг: 18 провалов подряд, минимум 9 с, медиана 36 с,
    # максимум 255 с, короче 3 секунд — ни одного. Тревоги были настоящими,
    # интернет действительно пропадал. Вернул 1,5 с: лишний запас не давал
    # ничего, а обнаружение реального обрыва замедлял вдвое.
    #
    # ПРАВИЛО: прежде чем трогать порог обнаружения события, измерь, сколько
    # это событие длится. Таймаутом не лечится то, что тянется минутами.
...
# ---------- Восстановление битых строк ДАТЧИКОВ ----------
#
# Линия башня → главная плата теряет байты: на CLC2 17.08 доля битых строк
# флагов и галетников была 61%, на CLC4 доходит до 90%. Корень — запрещённые
# прерывания при обновлении адресных лент и программный порт у башни Сов;
# это лечится отдельно и не за один вечер. Пока строка теряется, игрок ставит
# флаг и ничего не происходит — «сработало только со второго раза».
#
# ГРАНИЦА ПРОЩЕНИЯ. Восстанавливаем ТОЛЬКО отчёты датчиков о состоянии, и
# только когда догадка опирается на принятые буквы. Придумать событие, которого
# не было, хуже чем потерять: фантомный флаг засчитает этап, а фантомный restart
# убьёт партию. Поэтому:
#   1. кандидаты — только двадцать строк-состояний ниже, ничего с побочными
#      действиями; всё, что сервер уже знает, не трогаем вовсе;
#   2. «включён/выключен» решает _tail_state() по буквам f и n, а не близость
#      строк: «_on» короче «_off», и любой обрывок к нему ближе;
#   3. подходит больше одного кандидата — молчим. Так отсекается битый номер
#      флага: garbled «flag?_on» одинаково близок ко всем четырём;
#   4. каждое восстановление пишем в журнал — молча подменять нельзя.
#
# Правило проверено на 76 893 строках из журнала CLC4 за 10 дней: из 422 битых
# восстановлено 172 (40%), ни одна из 150 живых команд не перехвачена.
# Кандидаты берём из того же описания, что и _STATE_LINE_RE ниже: списки должны
# жить в одном месте, иначе разъедутся. Двадцать строк — пять приставок башен
# плюс центральный галетник, и четыре флага.
SENSOR_STATE_LINES = tuple(
    f"{prefix}galet_{state}"
    for prefix in ("", "workshop_", "basket_", "dog_", "owls_", "central_")
    for state in ("on", "off")
) + tuple(
    f"flag{n}_{state}" for n in (1, 2, 3, 4) for state in ("on", "off")
)
_sensor_recover_stats = {"ok": 0, "ambiguous": 0}
# Команды башен, опасно близкие к строкам-состояниям: пять галетников,
# каждый со своим номером.
_KNOWN_TOWER_LINE_RE = re.compile(r"galet[1-5](_off)?$")


def _edit_distance(a, b):
    """Расстояние Левенштейна. Строки короткие (8-10 символов), считаем в лоб."""
...
def _tail_state(raw):
    """Что говорит ХВОСТ строки: 'on', 'off' или None, если не разобрать.

    Разбор on/off нельзя доверять расстоянию Левенштейна: «_on» короче «_off»
    на символ, поэтому у любой оборванной строки он ближе, и «galet_» уехало бы
    в «галетник включён», а «flag4_˙5» — в «флаг поставлен». Фантомный флаг
    засчитает этап, которого не было. Поэтому состояние берём из фактически
    принятых букв: 'f' против 'n' в последних четырёх символах.
    """
    tail = raw[-4:]
    has_f, has_n = "f" in tail, "n" in tail
    if has_f and not has_n:
        return "off"
    if has_n and not has_f:
        return "on"
    return None
...
def recover_sensor_line(raw):
    ...
    # исправную команду в чужую.
    if raw in EVENT_DESCRIPTIONS or _KNOWN_TOWER_LINE_RE.match(raw):
        return None
    ...
    def unique(found, need_tail=True):
        ...
        if len(found) == 1:
            return found[0]
        if len(found) > 1:
            _sensor_recover_stats["ambiguous"] += 1
        return None
    ...
    # 2. Слипшиеся строки: внутри мусора лежит целая команда. Считаем только
    #    самые длинные совпадения, иначе «owls_galet_on» подменится на
    #    «galet_on» — а это РАЗНЫЕ команды, центральный галетник против башни.
    inside = [c for c in SENSOR_STATE_LINES if c in raw]
    inside = [c for c in inside if not any(c != o and c in o for o in inside)]
    if inside:
        return unique(inside, need_tail=False)
...
def _handle_dog_flash(dog_dev=None):
    ...
    try:
        ...
        # Шаг 3 не прошёл — на главной плате остался silence.ino или недописанная
        # прошивка. Это тихая беда: квест просто не работает, а причина неочевидна.
        # Кричим об этом прямо и заметно.
        if not ok3:
            logger.error("[FLASH] Dog: восстановление главной платы НЕ УДАЛОСЬ — требуется прошить main")
            socketio.emit('flash_log', {'board': 'dog', 'msg':
                '\n[SYSTEM] ‼️ ГЛАВНАЯ ПЛАТА НЕ ВОССТАНОВЛЕНА.\n'
                '[SYSTEM] На ней сейчас служебная прошивка silence или недописанная — квест работать не будет.\n'
                '[SYSTEM] Прямо сейчас нажмите «Прошить» у Главной Платы и дождитесь успеха.\n'})
            socketio.emit('flash_failed', {'board': 'main'})
...
DEVICE_COMMANDS = {
    ...
    'wolf': ('confirm_wolf_end', 'day_off', 'day_on', 'firework', 'game', 'ghost_game', 'ghost_game_end', 'language_1', 'language_2', 'language_3', 'language_4', 'language_5', 'language_6', 'open_door', 'ready', 'restart', 'skip', 'start', 'volume_down', 'volume_up', 'wolf_mode_easy', 'wolf_mode_normal',),
...
def sensor_monitor_set(data):
    ...
    # 22.08.2026. Башни отдают полный снимок входов ТОЛЬКО на переходе
    # «монитор выключен → включён». А главная плата повторяет mon:1 каждые
    # 5 секунд, поэтому башня часто считает монитор уже включённым: перехода
    # нет, снимка нет, и панель показывает лишь то, что успело шевельнуться.
    # Так на CLC4 «Локер клетки» не загорался при физически защёлкнутой
    # платформе — датчик исправен, просто его состояние никто не переспросил.
    # Ошибка вредная: по такой панели чинят исправное железо.
    # Лечим принудительным переходом: сначала выключаем, потом включаем.
    if sensor_monitor_active:
        serial_write_queue.put('mon:0')
        eventlet.sleep(0.2)
        serial_write_queue.put('mon:1')
    else:
        serial_write_queue.put('mon:0')
...
def handle_connect():
    ...
    # Состояние переключателя режима Волка (включён = лёгкий)
    socketio.emit('wolf_easy_state', wolf_easy_mode, to=request.sid)
...
def play_story(audio_source, loops=0, volume_file='3.txt'):
    ...
    try:
        ...
        if not story_fade_active and go == 1:
            ...
            temp_vol = phoneLevel
            # Приглушаем ОТНОСИТЕЛЬНО выставленной громкости, а не до постоянных
            # 10%. Раньше цель была абсолютной, и на первых делениях пульта фон
            # не приглушался вообще: деление 1 — это 5%, деление 2 — ровно 10%,
            # и цикл «пока громче цели» не делал ни одного шага. Звучало это как
            # «история идёт, а музыка её перекрикивает».
            target_vol = round(max(0.0, phoneLevel * STORY_DUCK_RATIO), 2)
...
def resend_sound_idle_state():
    ...
    2026-08-15, CLC4: посреди этапа Мастерской перестали работать подсказки —
    и только они, всё остальное шло как ни в чём не бывало. Плата снимает
    flagSound сама, когда отдаёт подсказку, а возвращает его ТОЛЬКО строка
    soundoff. Она пришла в 14:50:10 и, судя по всему, до платы не доехала:
    следующие пятнадцать минут ни один запрос подсказки не отработал, хотя
    огонь в печке и флаги от той же башни доходили.

    Состояние, живущее на одном фронте, рано или поздно рассинхронизируется —
    ровно та же болезнь, что была с флагами и кнопками пульта. Лечим тем же:
    повторяем состояние. Команда идемпотентная, на плате это просто
    flagSound = 1, а повтор раз в 20 секунд не заметен на фоне обмена.

    Важно: повторяем ТОЛЬКО когда канал историй свободен. Пока история или
    подсказка звучит, флаг обязан быть снят, иначе следующий запрос перебьёт
    текущую реплику."""
    global _sound_idle_resent_at
    if go != 1 or story_fade_active or story_busy():
        return
    now = time.time()
...
def serial():
     ...
     while True:
          try:
              ...
              resend_sound_idle_state() # Страховка от потерянного soundoff, см. функцию
              process_serial_queue()
              # --- МОНИТОРИНГ СЕРВЕРА ---
              ...
              for line in serial_lines:
                   ...
                   # --- ВОССТАНОВЛЕНИЕ БИТЫХ СТРОК ДАТЧИКОВ ---
                   # Обычный восстановитель ниже флаги и галетники намеренно
                   # не трогает, а теряются в первую очередь именно они: игрок
                   # ставит флаг, строка не доходит, и он ставит второй раз.
                   if isinstance(flag, str):
                       guess = recover_sensor_line(flag)
                       if guess:
                           _sensor_recover_stats["ok"] += 1
                           logger.info(f"ВОССТАНОВЛЕНА строка датчика: {flag!r} -> {guess}")
                           flag = guess
                   ...
                   if flag == "QUEST_SYSTEM_READY":
                       ...
                       send_boy_servo_calibration()
                       # Плата при перезагрузке забывает про монитор датчиков, а
                       # тумблер на пульте остаётся включённым — панель молча
                       # замирает, и человек проверяет датчики вслепую. Ровно так
                       # и вышло на CLC4 10.08. Состояние живёт на сервере,
                       # поэтому пересылаем его после каждой загрузки Mega.
                       if sensor_monitor_active:
                           serial_write_queue.put('mon:1')
                           logger.info("SETUP: монитор датчиков переотправлен после загрузки Mega")
                        ...
                        #-------закончили игру с флагами
                         ...
                         if flag=="win":
                              ...
                              if 'win_player' in socklist:
                                  continue
                              # HUE: гасим свет В МОМЕНТ ПОБЕДЫ.
                              # 2026-08-17: раньше гашение стояло ниже, вместе с
                              # салютом — то есть после двух голевых звуков и их
                              # ожиданий, от пары секунд до шестнадцати после
                              # самой победы. Всё это время комната оставалась
                              # освещённой на уровне предыдущего этапа, и начало
                              # победной истории проходило при свете.
                              # Свет на 100% по окончании story_66 ставит фоновая
                              # задача ниже — порядок OFF → 100% сохраняется, теперь
                              # с ещё большим запасом.
                              hue_light_async(False)
                              # --- ОТМЕНА ТАЙМЕРА ---
                              if basket_timeout_timer:
                                  basket_timeout_timer.cancel()
                                  basket_timeout_timer = None
                              ...
                              # HUE: свет здесь больше не трогаем — он погашен в
                              # начале ветки, в момент прихода win.
...
@socketio.on('toggle_wolf_easy')
def handle_wolf_easy_toggle(is_checked):
    """Лёгкий режим этапа Волка (включён = лёгкий, как у Чемоданов).
    ...
    global wolf_easy_mode
    new_state = bool(is_checked)
    if new_state == wolf_easy_mode:
        return  # повторный emit при reconnect
    _save_wolf_easy_preference(new_state)
    wolf_easy_mode = new_state
    _push_wolf_mode()
    logger.info(f"WOLF: режим {'ЛЁГКИЙ (луну можно отпускать)' if new_state else 'НОРМАЛЬНЫЙ (откат луны 5с)'}")
    socketio.emit('wolf_easy_state', new_state, to=None)
