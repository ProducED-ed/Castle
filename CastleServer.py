...
def gather_system_status():
    ...
    # Интернет — TCP-connect к 1.1.1.1:53 (eventlet-aware, цикл не блокирует).
    #
    # 19.08.2026: было 1,5 с. На проводном канале этого хватало, а на раздаче с
    # телефона сотовый модем после простоя поднимает радио 1-2 секунды, и проба
    # не укладывалась. Журнал CLC4 за 18.08 забит парами «Internet went OFFLINE /
    # restored» каждые полминуты весь день — при живой ассоциации, сильном
    # сигнале и нулевых ошибках в эфире, то есть тревоги ложные. Настоящие
    # обрывы в таком потоке не разглядеть.
    #
    # 3 секунды укладываются в цикл опроса (5 с) и переживают подъём радио.
    # Порог OFFLINE_FAIL_THRESHOLD = 2, значит о пропаже интернета сервер
    # сообщит примерно через 10-16 секунд — для индикатора это нормально,
    # игра от интернета не зависит вовсе.
    networks['internet'] = _check_tcp('1.1.1.1', 53, timeout=3.0)
