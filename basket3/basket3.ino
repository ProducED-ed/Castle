...
void HandleMessagges(String message) {
  ...
  if (message == "restart" || message == "ready") {
      ...
      if (message == "ready") {
          ...
          // 2026-08-15: приём restart башня не логировала вообще (sendLog выше
          // его намеренно пропускает), и по журналу нельзя было отличить
          // «команда не дошла» от «дошла, но не сработала». Ровно этот вопрос
          // встал, когда дверь в рестарте пульсировала как в игре.
          Serial1.println("log:basket:restart applied");
      }
      return;
  }
  ...
  //     подтвердил: норма только для START-режима, не для restart).
    ...
    basketDoorAllowed = false;  // тоже команда режима рестарта, см. open_door
    // 2026-08-22, CLC1: мало снять флаг двери баскетбола — пульсацию ТРОЛЛЕВОЙ
    // двери держит game-state (em1Active = state 3..7), а его сбрасывал только
    // сам restart. Если та строка терялась (на CLC1 главная плата зависает в
    // цикле до 6,3 с — worst_us=6292868, ровно как в инциденте CLC2 15.08),
    // state оставался игровым, и дверь толкало каждые 3 секунды в покое.
    // Эта команда приходит ТОЛЬКО в режиме рестарта, значит игра точно не идёт.
    state = 0;
    OpenLock(SHERIF_EM1);
  }
  ...
  if(message == "open_door") {
    // 2026-08-15, CLC2: у клиента дверь в режиме рестарта толкалась каждые
    // 3 секунды, как в игре. Флаг keep-alive снимался ТОЛЬКО по restart/ready,
    // а та строка потерялась: рестарт пришёлся на окно, когда главная плата
    // почти 6 секунд никого не слушала (LOOP worst_us=5928108). Флаг остался
    // поднятым, и DoorDefender продолжил пульсировать в покое.
    // open_door и open_mine_door сервер шлёт ТОЛЬКО в режиме рестарта
    // (go 0/2/3, см. CastleServer: ветка «В режиме РЕСТАРТА» и авто-открытие
    // дверей через 3 секунды после Restart). Значит их приход — сам по себе
    // достоверный признак, что игра не идёт, и keep-alive пора разоружить.
    // Второй путь к тому же результату: даже если restart потеряется, эта
    // команда придёт следом и вылечит.
    basketDoorAllowed = false;
    // Тот же довод, что и в open_mine_door: команда приходит только в рестарте,
    // поэтому разоружаем ОБА механизма пульсации, а не один. Иначе потерянный
    // restart оставлял троллеву дверь толкающейся.
    state = 0;
    OpenLock(SHERIF_EM2);
  }
